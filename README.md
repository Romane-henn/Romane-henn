## 👋 Salut !

Je m'appelle **Romane Hennnuyer**, j'ai 19 ans.

Je suis étudiante en **DEUST WMI** (*Webmaster et Métiers de l’Internet*) à l’ULCO de Calais.

Passionnée par le **graphisme**, la **communication** et le **web**, j’aime créer, imaginer et donner vie à des projets visuels.  
Mon objectif : travailler dans un métier créatif qui mêle **design**, **web** et **communication digitale**.

---

## 🎓 Mon parcours

- Baccalauréat obtenu en **juin 2024**  
  **Spécialités :**  
  - Mathématiques  
  - Physique-Chimie  
  - NSI *(Numérique et Sciences Informatiques – suivie en première)*

- Depuis 2024 : **DEUST WMI**  
  J’y apprends :  
  - la création de sites web  
  - l’intégration HTML/CSS  
  - la communication visuelle  
  - le travail sur des projets multimédias  
  - la gestion de contenus et d’identité numérique  

---

## 🛠️ Compétences

### 🎨 Graphisme  
- Création d’affiches  
- Logos & branding simple  
- Mise en page visuelle  
- Retouche et montage photo  
**Logiciels maîtrisés :** Photoshop, Illustrator, Canva

### 🌐 Web  
- HTML, CSS  
- Initiation à JavaScript  
- WordPress  

### 📣 Communication  
- Rédaction de contenus  
- Création de visuels pour réseaux sociaux  

---

## 💡 Ce que j’aime faire

- Créer des visuels 
- Concevoir des identités graphiques  
- Améliorer une mise en page  
- Imaginer des interfaces web  
- Monter des projets créatifs  
- Tester de nouvelles idées et apprendre en continu

---

## 🎯 Objectifs

- Devenir **graphiste**, **webdesigner** ou **chargée de communication digitale**  
- Développer davantage mes compétences en design et en développement web  
- Construire un portfolio professionnel unique et créatif

---

<!--
**Romane-henn/Romane-henn** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
